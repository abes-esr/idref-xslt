...
<!--     XSL de transformation du marc21Xml en marcXml Sudoc.
    ...
    v 20201008
  -->
<xsl:stylesheet exclude-result-prefixes="srw  xsi xs" version="2.0"
xmlns:srw="http://www.loc.gov/zing/srw/"
            ...
            <soap:Body>
                <ucp:updateRequest xmlns:srw="http://www.loc.gov/zing/srw/"
                    ...
                    <srw:record>
                        ...
                        <srw:recordData>
                            <xsl:choose>
                                <xsl:when test="mx:record/mx:controlfield[@tag='008']"  exclude-result-prefixes="mx" xmlns:mx="http://www.loc.gov/MARC21/slim">
                                    <xsl:apply-templates select="mx:record" mode="marc21" exclude-result-prefixes="mx" xmlns:mx="http://www.loc.gov/MARC21/slim" />                                                                       
                                </xsl:when>
                                <xsl:otherwise>  
                                    <xsl:apply-templates select="mx:record" mode="unimarc" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/></xsl:otherwise>
                            </xsl:choose>                     
                        </srw:recordData>
                    </srw:record>
                </ucp:updateRequest>
    ...
    <!-- ********** Marc 21 *************-->
    <xsl:template match="mx:record" mode="marc21" exclude-result-prefixes="mx" xmlns:mx="http://www.loc.gov/MARC21/slim">        
        <record >
        ...
        <datafield ind1="#" ind2="#" tag="200">
            <subfield code="a">
                <xsl:value-of select="mx:subfield[@code = 'a']"  exclude-result-prefixes="mx" xmlns:mx="http://www.loc.gov/MARC21/slim"/>  
            </subfield>
            <subfield code="b">
                <xsl:value-of select="mx:subfield[@code = 'q']"  exclude-result-prefixes="mx" xmlns:mx="http://www.loc.gov/MARC21/slim"/>
            </subfield>
            <subfield code="f">
                <xsl:value-of select="mx:subfield[@code = 'd']"  exclude-result-prefixes="mx" xmlns:mx="http://www.loc.gov/MARC21/slim"/>  />
            </subfield>
        </datafield>
    </xsl:template>
    ...
    <!-- ********** Unimarc *************-->
    <xsl:template match="mx:record" mode="unimarc"  exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
        <!--ERM le 24/06/20 -->
        ...
        <xsl:variable name="arbre1">
            <record>
                ...
                <xsl:for-each select="mx:datafield[@tag = '010'][mx:subfield[@code = 'a']][1]">
                    <datafield ind1="#" ind2="#" tag="010">
                        <xsl:apply-templates select="mx:subfield[@code = 'a']" mode="unimarc"/>
                        <xsl:choose>
                            ...
                            <xsl:otherwise>
                                <xsl:apply-templates select="mx:subfield[@code = '2']" mode="unimarc"/>
                            </xsl:otherwise>
                        </xsl:choose>
                        <xsl:for-each select="mx:subfield[string(@code) &gt; 'a']">
                            <xsl:sort order="ascending" select="string(@code)"/>
                            <xsl:apply-templates select="." mode="unimarc"/>
                        </xsl:for-each>
                    </datafield>
                </xsl:for-each>
                ...
                <xsl:apply-templates
                        select="mx:controlfield[not(@tag = '001' or @tag = '003' or @tag = '005')]" mode="unimarc"/>
             ...
             <xsl:apply-templates select="//mx:datafield[@tag = '102']"/>-->
                ...
                <xsl:for-each
                        select="mx:datafield[starts-with(@tag, '5') and (not(@tag = '510'))]">
                    <xsl:call-template name="zXXX">
                        <xsl:with-param name="zone" select="@tag"/>
                    </xsl:call-template>
                </xsl:for-each>
    ...
    <xsl:template name="zX10">
        ...
        <xsl:variable name="ind1">
            <xsl:choose>
                <xsl:when
                    test="string-length(normalize-space(//mx:datafield[@tag = '150']/mx:subfield[@code = 'a'])) = 1" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                    <xsl:text>0</xsl:text>
                </xsl:when>
                <xsl:when
                    test="string-length(normalize-space(//mx:datafield[@tag = '150']/mx:subfield[@code = 'b'])) = 1" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                    <xsl:choose>
                        <xsl:when
                            test="normalize-space(//mx:datafield[@tag = '150']/mx:subfield[@code = 'b']) = '0'" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                            <xsl:text>0</xsl:text>
                        </xsl:when>
                        <xsl:when
                            test="normalize-space(//mx:datafield[@tag = '150']/mx:subfield[@code = 'b']) = '1'" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                            <xsl:text>1</xsl:text>
                        </xsl:when>
                    </xsl:choose>
                </xsl:when>
                <xsl:when
                    test="mx:subfield[@code = 'd'] != '' or mx:subfield[@code = 'e'] != '' or mx:subfield[@code = 'f'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                    <xsl:text>1</xsl:text>
                </xsl:when>
                <xsl:otherwise>
    ...
    <xsl:template name="z033">
        ...
        <datafield ind1="#" ind2="#" tag="033">
            <subfield code="a">
                <xsl:value-of select="normalize-space(//mx:controlfield[@tag = '003'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            </subfield>
    ...
    <xsl:template name="z035">
        <datafield ind1="#" ind2="#" tag="035">
            <subfield code="a">
                <xsl:value-of select="normalize-space(//mx:controlfield[@tag = '001'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            </subfield>
        </datafield>
    </xsl:template>
    ...
    <xsl:template name="z103">
        <xsl:if test="//mx:datafield[@tag = '103']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
            <xsl:variable name="z103a">
                <xsl:value-of select="//mx:datafield[@tag = '103']/mx:subfield[@code = 'a']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            </xsl:variable>
            <xsl:variable name="z103b">
                <xsl:value-of select="//mx:datafield[@tag = '103']/mx:subfield[@code = 'b']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            </xsl:variable>
    ...
    <xsl:template name="z128">
        <xsl:variable name="z128a"
            select="normalize-space(//mx:datafield[@tag = '128']/mx:subfield[@code = 'a'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
        <xsl:variable name="z128b" select="//mx:datafield[@tag = '128']/mx:subfield[@code = 'b']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
        <xsl:variable name="z128c" select="//mx:datafield[@tag = '128']/mx:subfield[@code = 'c']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
    ...
    <xsl:template name="z150">
        <xsl:variable name="z150a"
            select="normalize-space(//mx:datafield[@tag = '150']/mx:subfield[@code = 'a'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
        <xsl:variable name="z150b"
            select="normalize-space(//mx:datafield[@tag = '150']/mx:subfield[@code = 'b'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
    ...
    <xsl:template name="z106">
        ...
        <datafield ind1="#" ind2="#" tag="106">
            <xsl:variable name="z200szx"
                select="normalize-space(//mx:datafield[@tag = '200']/mx:subfield[@code = 'x'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            <xsl:variable name="z200szy"
                select="normalize-space(//mx:datafield[@tag = '200']/mx:subfield[@code = 'y'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            <xsl:variable name="z200szz"
                select="normalize-space(//mx:datafield[@tag = '200']/mx:subfield[@code = 'z'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            <xsl:choose>
                <xsl:when
                    test="(substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 1, 1) != '') and $leader09_008 = 'p' and ((not(//mx:datafield[@tag = '200']/mx:subfield[@code = 'x']) or //mx:datafield[@tag = '200']/mx:subfield[@code = 'x'] = '') and (not(//mx:datafield[@tag = '200']/mx:subfield[@code = 'y']) or //mx:datafield[@tag = '200']/mx:subfield[@code = 'y'] = '') and (not(//mx:datafield[@tag = '200']/mx:subfield[@code = 'z']) or //mx:datafield[@tag = '200']/mx:subfield[@code = 'z'] = ''))" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                ...
                <xsl:otherwise>
                    <subfield code="a">
                        <xsl:choose>
                            <xsl:when
                                test="substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 1, 1) != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:value-of
                                    select="(substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 1, 1))" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"
                                />
                            </xsl:when>
                    ...
                    <subfield code="b">
                        <xsl:choose>
                            <xsl:when
                                test="substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 2, 1) != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:value-of
                                    select="substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 2, 1)" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"
                                />
                            </xsl:when>
                            <xsl:otherwise>#</xsl:otherwise>
                        </xsl:choose>
                    ...
                    <subfield code="c">
                        <xsl:choose>
                            <xsl:when
                                test="substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 3, 1) != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:value-of
                                    select="substring(normalize-space(//mx:datafield[@tag = '106']/mx:subfield[@code = 'a']), 3, 1)" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"
                                />
                            </xsl:when>
                            <xsl:otherwise>#</xsl:otherwise>
                        </xsl:choose>
                ...
                <xsl:when test="$mode = 'concat'">
                    <xsl:choose>
                        <xsl:when test="$tagSource = '301'">
                            <xsl:if test="mx:subfield[@code = 'a'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:text>Lieu de naissance : </xsl:text>
                                <xsl:call-template name="concatZ3XX">
                                    <xsl:with-param name="sousZone" select="'a'" />
                                </xsl:call-template>
                            </xsl:if>
                            <xsl:if test="mx:subfield[@code = 'b'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                        ...
                        <xsl:when test="$tagSource = '304'">
                            ...
                            <xsl:if test="mx:subfield[@code = 'd'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:text>Dates d’activité : </xsl:text>
                                <xsl:call-template name="concatZ3XX">
                                    <xsl:with-param name="sousZone" select="'d'"/>
                                </xsl:call-template>
                            </xsl:if>
                            <xsl:if test="mx:subfield[@code = 'l'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:text>Adresse : </xsl:text>
                                <xsl:call-template name="concatZ3XX">
                                    <xsl:with-param name="sousZone" select="'l'" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
                                </xsl:call-template>
                            </xsl:if>
                            <xsl:if test="mx:subfield[@code = 'e'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:text>Enseigne : </xsl:text>
                                <xsl:call-template name="concatZ3XX">
                                    <xsl:with-param name="sousZone" select="'e'" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
                                </xsl:call-template>
                            </xsl:if>
                        </xsl:when>
                        <xsl:when test="$tagSource = '341'">
                            ...
                            <xsl:if test="mx:subfield[@code = 'c'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:text>Adresse : </xsl:text>
                                <xsl:call-template name="concatZ3XX">
                        ...
                        <xsl:when test="$tagSource = '349'">
                            <xsl:if test="mx:subfield[@code = 'r'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                <xsl:text>Nature de la date ou des dates : </xsl:text>
                                <xsl:call-template name="concatZ3XX">
                                    <xsl:with-param name="sousZone" select="'r'"/>
                                </xsl:call-template>
                            </xsl:if>
                            <xsl:if test="mx:subfield[@code = 'p'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                            ...
                            <xsl:if test="mx:subfield[@code = 't'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                        ...
                        <xsl:when test="$tagSource = '352'">
                            ...
                            <xsl:if test="mx:subfield[@code = 'g'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                        ...
                        <xsl:when test="$tagSource = '360'">
                            ...
                            <xsl:if test="mx:subfield[@code = 'f'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                        ...
                        <xsl:when test="$tagSource = '361'">
                            ...
                            <xsl:if test="mx:subfield[@code = 'n'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
    ...
    <xsl:template name="concatZ3XX">
        ...
        <xsl:for-each select="mx:subfield[@code = $sousZone][text() != '']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
    ...
    <xsl:template name="zX10_zXXX">
        <xsl:param name="zone"/>
        <xsl:for-each select="mx:subfield[(string(@code) &lt; 'a') and (@code != '3')]" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
            <xsl:sort order="ascending" select="number(@code)"/>
            <xsl:choose>
                <xsl:when test="starts-with($zone, '5')">
                    <xsl:apply-templates select=".[not(@code = '9')]" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
                </xsl:when>
                <xsl:otherwise>
        ...
        <xsl:for-each select="mx:subfield[@code = '3']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
            <subfield code="Q">
                <xsl:value-of select="."/>
            </subfield>
        </xsl:for-each>
        <xsl:for-each select="mx:subfield[string(@code) &gt;= 'a']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
            <xsl:sort order="ascending" select="string(@code)"/>
            <xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when test=".[@code = 'a'] != ''">
                            <subfield code="a">
                                ...
                                <xsl:if
                                    test="./parent::mx:datafield/mx:subfield[@code = 'b'] != ''" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
                                    <xsl:value-of
                                        select="concat(', ', ./parent::mx:datafield/mx:subfield[@code = 'b'])" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"
                                    />
                                </xsl:if>
    ...
    <xsl:template match="mx:subfield[@code = '5']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
        ...
        <xsl:variable name="zone">
            <xsl:value-of select="./parent::mx:datafield/@tag" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
        </xsl:variable>
    ...
    <xsl:template match="mx:subfield[@code = '7']" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
    ...
    <xsl:template match="mx:subfield[@code = '8']" mode="unimarc" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
        <xsl:choose>
            <xsl:when test="./parent::mx:datafield[starts-with(@tag, '5')] or text() = 'frefre' or text() = 'fre   ' or text() = 'fre ' or contains(.,'|||')" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2"/>
            <xsl:otherwise>
                <subfield code="8">
                    <xsl:value-of select="."/>
                </subfield>
    ...
    <xsl:template match="mx:subfield[@code = '9']" mode="unimarc" exclude-result-prefixes="mx"  xmlns:mx="info:lc/xmlns/marcxchange-v2">
        <xsl:choose>
