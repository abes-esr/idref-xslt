...
<xsl:stylesheet exclude-result-prefixes="srw mx mxc xsi xs" version="2.0"
    ...
    <xsl:variable name="g_carNonTri">de De des Des du Du l' L' le Le la La [la] [La] les Les un Un
        une Une die Die (die) (Die) das Das the The [the] [The] los Los las Las el El uno Uno una
        Una il Il gli Gli </xsl:variable>
    <xsl:template match="/">
    ...
    <xsl:template name="z033">
        ...
        <xsl:if test="//mx:controlfield[@tag = '009'] != ''">
    ...
    <xsl:template name="typeAut">
        ...
        <xsl:variable name="z2XX">
            <xsl:value-of select="//mx:datafield[substring(@tag, 1, 1) = '2']/@tag"/>
        </xsl:variable>
        <xsl:choose>
                    ...
                    <xsl:choose>
                        <xsl:when test=".[@code = 'a'] != ''">
                            <subfield code="a">
                                ...
                                <xsl:if test="./parent::mx:datafield/mx:subfield[@code = 'b'] != ''">
                                    <xsl:value-of
    ...
    <!--ERM sept 22 ajout du template z856 pour traiter l'erreur : La sous-zone '$e' n'est pas répétable en 856 le problème implique aussi la $u. 
                ...
                <xsl:choose>
                    <xsl:when test="$nbCaraInitial = 1 and $fin = 'non'">
                        ...
                        <xsl:call-template name="tableau">
                            ...
                            <xsl:with-param name="sz5Initial" select="$sz5Initial"/>
                            <xsl:with-param name="nbCaraInitial" select="$nbCaraInitial"/>
                    ...
                    <xsl:when test="$fin = 'oui' and $generique = 'non'">
                        ...
                        <xsl:call-template name="tableau">
                            ...
                            <xsl:with-param name="sz5">
                                <xsl:choose>
                                    <xsl:when test="$nbCaraInitial = 1">
                                        <xsl:value-of select="$sz5Initial"/>
                                    </xsl:when>
                                    <xsl:otherwise>
                                        <xsl:value-of select="concat('xx', replace($sz5, 'x', ''))"
                                        />
                                    </xsl:otherwise>
                                </xsl:choose>
                            </xsl:with-param>
